...
class Sat():
    def __init__(self, params=None, type='kep', t0=JD2000, mu='geo'):
        self.t0 = t0
        if mu == 'geo':
            self.mu = mu_geo  # км/с
        elif mu == 'solar':
    ...
    def cartesian(self, t=JD2000, dt=None):
        ...
        q = np.array([a * (cos(E) - e), a * sqrt(1 - e ** 2) * sin(E), 0])
        dq = np.array([-a * n * sin(E) / (1 - e * cos(E)), a * sqrt(1 - e ** 2) * cos(E) * n / (1 - e * cos(E)), 0])

        q = R @ q.T  # T - преобразование строки к столбцу, суть транспозиция
        dq = R @ dq.T

        self.x, self.y, self.z = q.ravel()
        self.vx, self.vy, self.vz = dq.ravel()
